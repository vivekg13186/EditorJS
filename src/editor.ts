type Line = string[];
type Lines = Line[];
type Pos = { lno: number; col: number };
type Select = { start: Pos; end: Pos };
...
export class EditDoc {
  ...
  reachedEndOfDoc(): boolean {
    return (
      this.lno == this.lines.length - 1 &&
      this.col == this.lines[this.lno].length
    );
  }

  reachedStartOfDoc(): boolean {
    return this.lno == 0 && this.col == 0;
  }

  reachedEndOfLine(): boolean {
    return this.col == this.lines[this.lno].length;
  }
  reachedStartOfLine(): boolean {
    return this.col == 0;
  }
  putChar(c: string) {
  ...
  _updateSelectionAfter(shiftKey: boolean) {
    if (shiftKey) {
      if (this.selectionOn) {
        this.select.end.lno = this.lno;
        this.select.end.col = this.col;
  ...
  copyArray(arr: Line, from: number, to: number): Line {
    var result: Line = [];
